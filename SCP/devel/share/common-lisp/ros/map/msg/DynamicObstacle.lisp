...
(cl:defclass <DynamicObstacle> (roslisp-msg-protocol:ros-message)
  ((polygon
   ...
   (trajectory
    :reader trajectory
    :initarg :trajectory
    :type (cl:vector map-msg:DynamicTrajectoryPoint)
   :initform (cl:make-array 0 :element-type 'map-msg:DynamicTrajectoryPoint :initial-element (cl:make-instance 'map-msg:DynamicTrajectoryPoint))))
)
...
(cl:defmethod cl:initialize-instance :after ((m <DynamicObstacle>) cl:&rest args)
  ...
  (cl:unless (cl:typep m 'DynamicObstacle)
    (roslisp-msg-protocol:msg-deprecation-warning "using old message class name map-msg:<DynamicObstacle> is deprecated: use map-msg:DynamicObstacle instead.")))

(cl:ensure-generic-function 'polygon-val :lambda-list '(m))
(cl:defmethod polygon-val ((m <DynamicObstacle>))
  (roslisp-msg-protocol:msg-deprecation-warning "Using old-style slot reader map-msg:polygon-val is deprecated.  Use map-msg:polygon instead.")
  (polygon m))

(cl:ensure-generic-function 'trajectory-val :lambda-list '(m))
(cl:defmethod trajectory-val ((m <DynamicObstacle>))
  (roslisp-msg-protocol:msg-deprecation-warning "Using old-style slot reader map-msg:trajectory-val is deprecated.  Use map-msg:trajectory instead.")
  (trajectory m))
...
(cl:defmethod roslisp-msg-protocol:deserialize ((msg <DynamicObstacle>) istream)
  ...
  (cl:let ((vals (cl:slot-value msg 'trajectory)))
    ...
    (cl:setf (cl:aref vals i) (cl:make-instance 'map-msg:DynamicTrajectoryPoint))
...
(cl:defmethod roslisp-msg-protocol:message-definition ((type (cl:eql '<DynamicObstacle>)))
  ...
  (cl:format cl:nil "geometry_msgs/Polygon polygon~%DynamicTrajectoryPoint[] trajectory~%~%================================================================================~%MSG: geometry_msgs/Polygon~%#A specification of a polygon where the first and last points are assumed to be connected~%Point32[] points~%~%================================================================================~%MSG: geometry_msgs/Point32~%# This contains the position of a point in free space(with 32 bits of precision).~%# It is recommeded to use Point wherever possible instead of Point32.  ~%# ~%# This recommendation is to promote interoperability.  ~%#~%# This message is designed to take up less space when sending~%# lots of points at once, as in the case of a PointCloud.  ~%~%float32 x~%float32 y~%float32 z~%================================================================================~%MSG: map/DynamicTrajectoryPoint~%float64 time~%float64 x~%float64 y~%float64 theta~%~%~%"))
(cl:defmethod roslisp-msg-protocol:message-definition ((type (cl:eql 'DynamicObstacle)))
  "Returns full string definition for message of type 'DynamicObstacle"
  (cl:format cl:nil "geometry_msgs/Polygon polygon~%DynamicTrajectoryPoint[] trajectory~%~%================================================================================~%MSG: geometry_msgs/Polygon~%#A specification of a polygon where the first and last points are assumed to be connected~%Point32[] points~%~%================================================================================~%MSG: geometry_msgs/Point32~%# This contains the position of a point in free space(with 32 bits of precision).~%# It is recommeded to use Point wherever possible instead of Point32.  ~%# ~%# This recommendation is to promote interoperability.  ~%#~%# This message is designed to take up less space when sending~%# lots of points at once, as in the case of a PointCloud.  ~%~%float32 x~%float32 y~%float32 z~%================================================================================~%MSG: map/DynamicTrajectoryPoint~%float64 time~%float64 x~%float64 y~%float64 theta~%~%~%"))
