...
private:
  ...
  bool GenerateSpeedAcceleration(HybridAStartResult *result); //获得速度加速度
  bool GenerateSCurveSpeedAcceleration(HybridAStartResult *result); //生产s曲线
...
private:
  ...
  std::unordered_map<std::string, std::shared_ptr<Node3d>>
      open_set_; //存放未遍历的节点节点
  std::unordered_map<std::string, std::shared_ptr<Node3d>>
      close_set_; //存放已经遍历的节点
