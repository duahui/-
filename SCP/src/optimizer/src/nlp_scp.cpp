...
namespace nlp{
void ipopt_interface::init_path_contigence(const vector<Eigen::Matrix<double, 7, 1>> &state_list, const vector<std::array<double, 8>> &boxes) 
{
    int a = 0;
}
/* API */
ipopt_interface::ipopt_interface(ros::NodeHandle &nh)
{
...
void ipopt_interface::init_path_info(const vector<Matrix<double, 6, 1>> &state_list, const vector<Matrix<double, 4, 1>> &bounds, const vector<Matrix<double, 4, 1>> &currents)
{        
    currents_info = std::move(currents);
    ...
    xy_bounds = std::move(bounds);

    num_of_variables = 4 * (horizon + 1) + 2 * horizon + 1;    //状态量 + 控制量 + 时间
    num_of_constraints = 4 * horizon + horizon;                         //动力学 + 方向盘变化率
    ...
    double max_= 0;
    // for (int i = 0; i < currents_info.size(); i++)
    // {
    //     max(max_, max(currents_info[i](2),currents_info[i](3)) );
    // }
    
    // max_speed_forward = 5.0 + max_; 
    max_speed_reverse = 0;
    // max_acc_forward = 3.0;
...
void ipopt_interface::get_opt_result(MatrixXd &state_result_, MatrixXd &control_result_, double &time_result_)
{
...
bool ipopt_interface::get_nlp_info(Index & n, Index & m, Index & nnz_jac_g, Index & nnz_h_lag, IndexStyleEnum & index_style)
{
    n = num_of_variables;
...
bool ipopt_interface::get_bounds_info(Index n, Number * x_l, Number * x_u, Index m, Number * g_l, Number * g_u)
{
    ...
    {
        // x
        x_l[variable_index] = xy_bounds[i](0);
        x_u[variable_index] = xy_bounds[i](1);

        // y
        x_l[variable_index + 1] = xy_bounds[i](2);
        x_u[variable_index + 1] = xy_bounds[i](3);
...
bool ipopt_interface::get_starting_point(Index n, bool init_x, Number * x, bool init_z, Number * z_L, Number * z_U, Index m, bool init_lambda, Number * lambda)
{
    /* 检测各标志位是否正确，默认只有 init_x  为 ture*/
    assert(init_x == true);
    assert(init_z == false);
...
bool ipopt_interface::eval_f(Index n, const Number * x, bool new_x, Number & obj_value)
{
    //由于 object_value 涉及到 ADOLC 的调用，所以需要额外套一层函数
    eval_obj(n, x, obj_value);

    return true;
}

bool ipopt_interface::eval_grad_f(Index n, const Number * x, bool new_x, Number * grad_f)
{
...
bool ipopt_interface::eval_g(Index n, const Number * x, bool new_x, Index m, Number * g)
{
...
bool ipopt_interface::eval_jac_g(Index n, const Number * x, bool new_x, Index m, Index nele_jac, Index * iRow, Index * jCol, Number * values)
{
    //ADOLC
    if (values == NULL) 
    {
...
bool ipopt_interface::eval_h(Index n, const Number * x, bool new_x, Number obj_factor, Index m, const Number * lambda, bool new_lambda, Index nele_hess, Index * iRow, Index * jCol, Number * values)
{
    //ADOLC
    if (values == NULL) 
    {
...
/* 获得优化的结果 */
void ipopt_interface::finalize_solution(SolverReturn status, Index n, const Number * x, const Number * z_L, const Number * z_U, Index m, const Number * g, const Number * lambda, Number obj_value, const IpoptData * ip_data, IpoptCalculatedQuantities * ip_cq)
{
    int state_index = 0;
...
template <typename T>  
void  ipopt_interface::eval_obj(Index n, const T *x, T& obj_value)
{
    int control_index = control_start_index;
    int state_index = 0;
    ...
    for (int i = 0; i < horizon + 1; ++i)
    {
        T vx = x[state_index + 3] * cos(x[state_index + 2]);
        T vy = x[state_index + 3] * sin(x[state_index + 2]);
        obj_value -= w_current * (vx * currents_info[i](2) + vy * currents_info[i](3));
...
template <class T>    
void ipopt_interface::eval_constraints(Index n, const T *x, Index m, T *g)
{
    int control_index = control_start_index;
    int state_index = 0;
    int constraint_index = 0;
...
void ipopt_interface::generate_tapes(Index n, Index m, Index& nnz_jac_g, Index& nnz_h_lag)
{
