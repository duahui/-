...
void PlotVehicle(int id, const math::Pose &pt, double phi) {
  ...
  int tire_id = 1;
  for (auto &tire: tires) {
    visualization::PlotPolygon(math::Polygon2d(tire), 0.1, visualization::Color::Grey, id * (tire_id++),
                                "Tires");
  }
  visualization::PlotPolygon(math::Polygon2d(config_.GenerateBox({pt.x(), pt.y(), pt.theta()})), 0.2,
                              visualization::Color::Yellow, id, "Footprint");
  visualization::Trigger();
}

std::array<math::Box2d, 4> GenerateTireBoxes(const math::Pose pose, double phi) {
  auto front_pose = pose.extend(config_.wheel_base);
  auto track_width = config_.width - 0.195;
