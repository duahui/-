...
#include "qp/linear_interpolation.h"
#include "qp/math_utils.h"
#include <cmath>
...
double slerp(const double a0, const double t0, const double a1, const double t1,
  ...
  const double a0_n = NormalizeAngle(a0); //规范化角度值-pi到pi
  const double a1_n = NormalizeAngle(a1);
  double d = a1_n - a0_n; //弧度差
  if (d > M_PI) {
    d = d - 2 * M_PI;
  } else if (d < -M_PI) {
    d = d + 2 * M_PI;
  ...
  const double r = (t - t0) / (t1 - t0); //弧长占比
  const double a = a0_n + d * r;         // a0_n+弧度占比
...
PathPoint InterpolateUsingLinearApproximation(const PathPoint &p0,
  ...
  double x = (1 - weight) * p0.x() + weight * p1.x(); // p0+w(p1-p0)
